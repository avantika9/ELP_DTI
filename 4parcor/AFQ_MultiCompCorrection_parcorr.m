function [alphaFWE, statFWE, clusterFWE, stats] = AFQ_MultiCompCorrection_parcorr(data,y,covariates,alpha,method,cThresh)
        ...
        switch(stattest)
            case('corr')
                for ii = 1:nperm
                    ...
                    [stat(ii,:) p(ii,:)] = partialcorr(y, data(rows,:), covariates,'rows', 'pairwise','Type','Spearman');
                    %[stat(ii,:) p(ii,:)] = partialcorr(y, data(rows,:), covariates,'rows', 'pairwise','Type','Pearson');
                end
            case('ttest')
                for ii = 1:nperm
                    rows = Shuffle(y);
